...
export const CalculatorPage: React.FC<Props> = (props: Props) => {
    ...
    let appleEquivalent = result / APPLE_CALORIES;
    const appleFloor = Math.floor(appleEquivalent);
    const appleCeil = Math.ceil(appleEquivalent);
    const appleCloseToInteger: boolean = (appleEquivalent - appleFloor < DIFF_THRESHOLD
        || appleCeil - appleEquivalent < DIFF_THRESHOLD);

    // if appleEquivalent is less than 0.2 away from the floor or ceil
    // round to the nearest integer
    if (appleCloseToInteger) {
        appleEquivalent = Math.round(appleEquivalent);
    }
        ...
        <div className={classes.container}>
            <div className={classes.calculatorContainer}>
                ...
                <div className={classes.calculatorInnerContainer}>
                    ...
                    <div className={classes.buttonContainer}>
                        <Button variant="contained" onClick={sendData(result)}>
                            {appleCloseToInteger ? <>{appleEquivalent} Apples</> : <>{appleFloor} to {appleCeil} Apples</>}
                        </Button>
                    </div>
                </div>
                <div className={classes.lateralBar}>
